...
simulateComp_snp <- function(madII, G.cor, E.cor, dirh2, compG.var, fra.CompE, plotSize){
  madII <- na.omit(madII)
...
#to get the effects for the dataset
  ...
  #competition incidence matrix 
   ##may be try without specifying the competition incidence?
  distMat.4 <- distMat
  ...
  competinfluence <- Z.comp.4 %*% compet
